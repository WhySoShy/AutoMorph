...
internal static class CollectionCasting
{
    ...
    internal static Dictionary<string, Func<ReferencePropertyToken, string, string>> SupportedCollections { get; } = new()
    {
        ...
        {BLOCKING_CONCURRENT_COLLECTION, (token, sourceReference) => $"new {GLOBAL_KEYWORD}{BLOCKING_CONCURRENT_COLLECTION}<{GLOBAL_KEYWORD}{token.NestedObject!.Type.Key}>({GetGenericCast(QUEUE_CONCURRENT_COLLECTION, token, sourceReference)})"},
    ...
    private static string GetGenericCast(string collectionTypeName, ReferencePropertyToken token, string sourceReference)
        => $"new {GLOBAL_KEYWORD}{collectionTypeName.Replace("<T>", $"<{GLOBAL_KEYWORD}{token.NestedObject!.Type.Key}>")}({sourceReference}.{token.SourcePropertyName}.{token.NestedObject!.MethodToken.Name})";
    
    private static string GetImmutableCast(string collectionTypeName, ReferencePropertyToken token, string sourceReference)
        => $"{GLOBAL_KEYWORD}{collectionTypeName}.CreateRange({sourceReference}.{token.SourcePropertyName}.{token.NestedObject!.MethodToken!.Name}())";
